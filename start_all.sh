...
API_HOST="${EVOLVE_AGENT_API_HOST:-0.0.0.0}"
API_PORT="${EVOLVE_AGENT_API_PORT:-8110}"
BASE_URL="${EVOLVE_AGENT_BASE_URL:-http://127.0.0.1:${API_PORT}}"
WAIT_TIMEOUT="${EVOLVE_AGENT_START_WAIT_TIMEOUT:-60}"
AGENT_LOG="${EVOLVE_AGENT_API_LOG:-logs/evolve-agent.log}"
AGENT_PID_FILE="${EVOLVE_AGENT_API_PID_FILE:-logs/evolve-agent.pid}"
...
if is_running_pid "$EXISTING_PID"; then
  echo "Evolve Agent API is already running (PID: $EXISTING_PID)"
  echo "Health: ${BASE_URL}/health"
  exit 0
fi

echo "Starting Evolve Agent API on ${API_HOST}:${API_PORT}..."
bash ./start_agent.sh >"$AGENT_LOG" 2>&1 &
AGENT_PID=$!
echo "$AGENT_PID" >"$AGENT_PID_FILE"

if ! wait_for_service "${BASE_URL}/health" "$WAIT_TIMEOUT" "$AGENT_PID"; then
  echo "Failed to start Evolve Agent API. Recent log output:" >&2
